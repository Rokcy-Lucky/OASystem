			...
			<span class="l"><a href="javascript:;" onclick="datadel()"
				...
				class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i>
					添加用户</a></span> <span class="r">共有数据：<strong><c:out
						value="${page1.totalCount }"></c:out></strong> 条
			</span>
		</div>
		<div class="mt-20">
			<table
				...
				<tbody>
					...
					<c:forEach items="${users1 }" var="userinfo">
						<tr class="text-c">
				...
				<div align="center">
					<font size="2">共 ${page1.totalPageCount} 页</font> <font size="2">第
						${page1.pageNow} 页</font> <a href="usersearch.do?pageNow=1&totalCount=${page1.totalCount }&username1=<%=request.getParameter("username1") %>">首页</a>
					<c:choose>
						<c:when test="${page1.pageNow - 1 > 0}">
							<a href="usersearch.do?pageNow=${page1.pageNow - 1}&totalCount=${page1.totalCount }&username1=<%=request.getParameter("username1") %>"">上一页</a>
						</c:when>
						<c:when test="${page1.pageNow - 1 <= 0}">
							<a href="usersearch.do?pageNow=1&totalCount=${page1.totalCount }&username1=<%=request.getParameter("username1") %>"">上一页</a>
						</c:when>
					</c:choose>
					<c:choose>
						<c:when test="${page1.totalPageCount==0}">
							<a href="usersearch.do?pageNow=${page1.pageNow}&totalCount=${page1.totalCount }&username1=<%=request.getParameter("username1") %>"">下一页</a>
						</c:when>
						<c:when test="${page1.pageNow + 1 < page1.totalPageCount}">
							<a href="usersearch.do?pageNow=${page1.pageNow + 1}&totalCount=${page1.totalCount }&username1=<%=request.getParameter("username1") %>"">下一页</a>
						</c:when>
						<c:when test="${page1.pageNow + 1 >= page1.totalPageCount}">
							<a href="usersearch.do?pageNow=${page1.totalPageCount}&totalCount=${page1.totalCount }&username1=<%=request.getParameter("username1") %>"">下一页</a>
						</c:when>
					</c:choose>
					<c:choose>
						<c:when test="${page1.totalPageCount==0}">
							<a href="usersearch.do?pageNow=${page1.pageNow}&totalCount=${page1.totalCount }&username1=<%=request.getParameter("username1") %>"">尾页</a>
						</c:when>
						<c:otherwise>
							<a href="usersearch.do?pageNow=${page1.totalPageCount}&totalCount=${page1.totalCount }&username1=<%=request.getParameter("username1") %>"">尾页</a>
						</c:otherwise>
					</c:choose>
				</div>
			</table>
